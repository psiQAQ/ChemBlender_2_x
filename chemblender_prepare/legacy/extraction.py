...
def extract_legacy_objects(detection: LegacySceneDetection | None = None) -> LegacyExtractionReport:
    ...
    if any(item.kind == "crystal" for item in detection.objects):
        from .cif_schema import legacy_cif_schema
        with legacy_cif_schema():
            return _extract_detected_objects(detection)
    return _extract_detected_objects(detection)


def _extract_detected_objects(detection):
    import bpy
