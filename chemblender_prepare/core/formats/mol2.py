...
def mol2_preview_summary(batch):
    """Summarize interpreted content without serializing scientific arrays."""
    annotations = {
        key: tuple(
            sorted(
                {
    ...
    return {
        ...
        "interpreted_bond_count": sum(
            topology.bond_indices.shape[0] for topology in batch.topologies
        ),
        "molecule_types": ", ".join(annotations["molecule_type"])[:1024],
        "charge_types": ", ".join(annotations["charge_type"])[:1024],
        ...
        "unsupported_sections": (
            ", ".join(unsupported_sections)[:1024] if unsupported_sections else "none"
        ),
    }
