    ...
    try:
        ...
        if error.name == "gemmi":
            raise GemmiDependencyError(
                "Gemmi is required in the external chemblender_prepare environment"
            ) from error
        raise
    return gemmi
...
def numeric_symmetry_operations(operations):
    """Convert explicitly supplied CIF triplets to paired fractional arrays.

    Gemmi stores both numerator matrices and translations with denominator DEN.
    Preserve input operation order and unwrapped translations; never select a
    space-group setting or manufacture an identity operation for absent data.
    """
    import numpy

    operations = tuple(operations)
    if not operations:
        raise ValueError("numeric symmetry requires explicit operation strings")
    gemmi = _gemmi()
    rotations = []
    translations = []
    for index, value in enumerate(operations):
        try:
            operation = gemmi.Op(value)
        except (RuntimeError, ValueError, TypeError) as error:
            raise ValueError(f"invalid symmetry operation {index}: {value!r}") from error
        numerator = numpy.asarray(operation.rot, dtype=numpy.int64)
        if numpy.any(numerator % operation.DEN):
            raise ValueError(f"symmetry operation {index} has a non-integral rotation")
        rotations.append(numerator // operation.DEN)
        translations.append(numpy.asarray(operation.tran, dtype=float) / operation.DEN)
    return (
        ArrayData(numpy.asarray(rotations), ("symmetry_operation", "row", "column"), "dimensionless"),
        ArrayData(numpy.asarray(translations), ("symmetry_operation", "xyz"), "dimensionless"),
    )
...
def _parse_block(
    ...
    *,
    ...
    rotations, translations = (
        numeric_symmetry_operations(symmetry_operations)
        if symmetry_operations else (None, None)
    )
