...
def project_conformer_batch(project):
    """Use persistent project identities when reviewing and accepting candidates."""
    return ImportBatch(structures=tuple(project.structures.values()),
        topologies=tuple(project.topologies.values()), molecular_records=tuple(project.molecular_records.values()),
        datasets=tuple(project.datasets.values()), provenance=tuple(project.provenance.values()),
