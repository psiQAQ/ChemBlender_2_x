"""External PubChem preparation and verified source provenance; no Blender API."""
...
from cbq_core.model import ImportBatch, ProvenanceRecord
...
def _fetch(url, *, timeout):
    # One response is bounded; a separate CLI cancellation path is still required.
    with urlopen(url, timeout=timeout) as response:
        content = response.read(64 * 1024 * 1024 + 1)
        if len(content) > 64 * 1024 * 1024:
            raise OSError("PubChem response exceeds 64 MiB")
        return SimpleNamespace(status_code=response.status, content=content,
    ...
    *,
    ...
    def check_cancelled():
        if is_cancelled is not None and is_cancelled():
            raise CancelledError("PubChem preparation cancelled")
    ...
    try:
        ...
        for path in reversed(created):
            try:
                ...
                error.add_note(f"Could not remove staged file {path}: {cleanup_error}")
        raise
    return PubChemImportStage(
        request=request,
        source_url=source_url,
