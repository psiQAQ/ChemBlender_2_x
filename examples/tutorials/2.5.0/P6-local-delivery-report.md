...
- 中英离线 HTML 各包含 71 张内嵌图片和一个真实 review ZIP 下载；其中包含 3 张 T04 当前可见状态/成图、3 张 T06 当前帧标量截图、4 张 T18 当前迁移/重链截图、4 张 T03 当前 SDF 分组/拒绝/Project Browser/F12 截图，以及 5 张 T05 当前 PDB/PQR/MOL2/F12 截图。此前 Edge 断网加载通过；本次资源图、锚点和下载完整性由生成器与离线测试复核。
...
- T00 run-010 已用真实 Windows 键鼠完成当前 ZIP 重装、Test Processor、无效 processor 路径报错、有效路径恢复和重试；两个 Blender PID 均正常退出。检查器为 `integrity_status=passed`、`technical_status=passed`，仅独立审阅未完成；review ZIP v2 的 66 个成员均为相对安全路径，38 张失败或非验收截图只位于 `failures/`。
- T06 当前候选侧栏已在 source frame 0/15/31 显示完整 Energy、Source Index、Step，三张可读截图和连续 Game Bar 操作录制均已按哈希保留；当前 Cycles 成图、无源且无 processor 的 REBUILD、独立进程冷重开和科学数组不变检查均通过，原 GUI 工程以“不保存”退出，字节未改变。

## 仍未关闭

- P0 当前 checker 结论是：T00/T01/T02/T04/T06 `integrity_status=passed`、`technical_status=passed`，但独立审阅未完成，所以总 verdict 仍为 `incomplete`；T06 的历史 invalid manifest 原样保留并由当前 replacement manifest 取代，T07/T17/T18 仍无 conforming manifest。完成审计并记录阻塞，不能替代案例技术验收。
