...
T00 diagnostics and T01 GUI conversion/import/View/render/save/cold reopen have run. Scientific checks and offline browser checks passed. The checker still reports `incomplete`: final visual review is pending, and no human independent replay has occurred. Later case progress and candidate-specific limitations are recorded in status.json and individual receipts; this checkpoint does not close the overall plan.
