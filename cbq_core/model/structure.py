...
from .common import _require_known_length_unit
from .common import _require_text
from .common import _require_uuid
from .common import _require_uuid_tuple
...
class PeriodicSiteData:
    ...
    symmetry_rotations: ArrayData | None = None
    symmetry_translations: ArrayData | None = None
    ...
    def __post_init__(self):
        ...
        _validate_numeric_symmetry(
            self.symmetry_rotations, self.symmetry_translations, len(symmetry_operations)
        )
...
def _validate_numeric_symmetry(rotations, translations, operation_count):
    """Validate paired fractional operations; source strings retain their order.

    Crystallographic rotation orders are 1, 2, 3, 4 or 6. Exact integer
    arithmetic also rejects unimodular shears without overflow or rounding.
    """
    import numpy

    if rotations is None and translations is None:
        return  # Historical packages remain readable until external upgrade.
    if not isinstance(rotations, ArrayData) or not isinstance(translations, ArrayData):
        raise ValueError("symmetry_rotations and symmetry_translations must be paired ArrayData")
    r = numpy.asarray(rotations.values)
    t = numpy.asarray(translations.values)
    if (
        rotations.dims != ("symmetry_operation", "row", "column")
        or rotations.shape != (operation_count, 3, 3)
        or rotations.unit != "dimensionless"
        or r.dtype.kind not in "iu"
    ):
        raise ValueError("symmetry_rotations must contain one integer dimensionless (3, 3) matrix per operation")
    if (
        translations.dims != ("symmetry_operation", "xyz")
        or translations.shape != (operation_count, 3)
        or translations.unit != "dimensionless"
        or t.dtype.kind not in "iuf"
        or not numpy.all(numpy.isfinite(t))
    ):
        raise ValueError("symmetry_translations must contain one finite dimensionless vector per operation")
    identity = numpy.eye(3, dtype=object)
    for rotation in r:
        exact = rotation.astype(object)
        a, b, c = exact
        determinant = (
            a[0] * (b[1]*c[2] - b[2]*c[1])
            - a[1] * (b[0]*c[2] - b[2]*c[0])
        ...
        if abs(determinant) != 1:
            raise ValueError("symmetry_rotations must be unimodular")
        if not any(
            numpy.array_equal(numpy.linalg.matrix_power(exact, order), identity)
            for order in (1, 2, 3, 4, 6)
        ):
            raise ValueError("symmetry_rotations must have crystallographic finite order")
