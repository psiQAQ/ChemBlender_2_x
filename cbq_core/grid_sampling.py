...
def _check_cancel(is_cancelled):
    if is_cancelled is not None:
        from .storage.text import ExportCancelled
        from .storage.text import _cancelled
...
def export_grid_sample(destination, grid, *, kind, settings, is_cancelled=None):
    ...
    from .storage.text import ExportReport
    from .storage.text import atomic_write_chunks
