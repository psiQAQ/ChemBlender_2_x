...
3. 核对分行显示的 shape、coordinate unit、semantic、value unit 和 quality，再设置 isovalue。`Threshold` 行用有效数字显示实际阈值，小于数字控件显示精度的值也不会只显示成零；阈值采用该 Grid 的 value unit。
...
重新打开文件后，选择对应 Structure 和 Coordinates，再点击 **Configure Trajectory Playback** 恢复播放。文件切换会关闭旧轨迹绑定；保存的 FrameSet 和科学数组保持不变。
