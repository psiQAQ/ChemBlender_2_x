...
Review-ready technical draft. Public conversion, five primary Grid Views created through the GUI, signed display, numerical sampling, rendering, serial cold/cache recovery and source/processor-unavailable recovery have evidence. Independent human replay remains pending.
...
Complete [installation](installation.md) and [the first lesson](first-aspirin.md). The direct-GUI record uses Blender 5.1.1, prepare 0.1.0 and accepted Extension SHA-256 `a1e2da79253d505b60daa42aa465eb725cdd1eba00e6c81ce4082102a62d0f28`. A later Phase 4 test candidate, SHA-256 `2637ce7ccc846854a280ebe56ea91f858a29591aa6614cc53cd92ea28bb3ea3f`, changes only the T06 read-only trajectory rows and separately passed this lesson's offline recovery; it is not relabeled as a new GUI run.
...
The verified processor route reused an existing scientific Python with pymatgen-core 2026.7.16 and the frozen prepare code. No dependencies were installed. This is `development_reuse`, not an isolated install or distribution-ready environment, because its import route is not self-contained. Qualify your [scientific route](../../prepare/en/advanced-routes.md) before selecting Reader ID `pymatgen-vasp-grid`; availability in Standard alone is not assumed. Public inspect/convert/validate passed on this route. The current accepted candidate has direct GUI evidence for the primary Cube route; the VASP variant has native public-Operator replay, render and recovery evidence only.
...
This figure shows a finite grid domain. Boundary appearance is affected by the sampled domain; do not interpret its outer shape as a physical crystal surface. PAW augmentation occupancies are not reconstructed here. The saved pair cold-reopened with local VDB paths and unchanged scientific arrays; a separate relocated copy cold-reopened successfully. Removing five derived VDB files only in that copy, rebuilding both Views and cold-reopening again passed, retaining the twelve cell annotation edges and all scientific arrays. The original pair stayed unchanged. See the [variant recovery receipt](../../../examples/tutorials/2.5.0/T07-vasp-recovery-check.json). VASP direct GUI and independent human review remain pending and are not inferred from native replay.
...
Rebuild restores the generated `Grid` resolution: reapply Size 0.025 and any custom material refinement afterward. The original refined project is unchanged. A separate H2 copy was opened with both the recorded Cube source and a deliberately nonexistent processor path unavailable. The load handler restored three derived VDBs from the local CBQ arrays; those VDBs were removed again, public `Rebuild Selected View` restored both Views, and an independent cold process reopened the saved pair with three local arrays and three local VDBs unchanged. The source was restored with its original hash. See the [offline recovery receipt](../../../examples/tutorials/2.5.0/T07-run010-offline-recovery-check.json). This is native replay evidence, not a GUI recovery recording. The local review package is review-only; independent human acceptance remains pending.
