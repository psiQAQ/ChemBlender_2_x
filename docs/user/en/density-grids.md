...
This figure shows a finite grid domain. Boundary appearance is affected by the sampled domain; do not interpret its outer shape as a physical crystal surface. PAW augmentation occupancies are not reconstructed here. The saved pair cold-reopened with local VDB paths and unchanged scientific arrays; A separate relocated copy cold-reopened successfully. Removing five derived VDB files only in that copy, rebuilding both Views and cold-reopening again passed, retaining the twelve cell annotation edges and all scientific arrays. The original pair stayed unchanged. See the [variant recovery receipt](../../../examples/tutorials/2.5.0/T07-vasp-recovery-check.json). GUI recovery, source/processor-unavailable recovery and human review remain pending.
