...
- [x] Record externally reviewed remote HEAD `32b26bb4508a29d3ac0763256869fb9e8daac5f4`.
- [x] Mark the previously authorized final-hardening push as complete.
- [x] Persist this gate as the only active execution cursor.
...
- [x] Write failing tests for sniff/parse `MemoryError`, plugin exceptions,
  incomplete or invalid exact public batches, and valid XYZ/Cube no-copy
  results.
- [x] Continue after ordinary sniff failures and return stable failed batches
  for ordinary parse/result-validation failures.
- [x] Re-raise `MemoryError` unchanged.
- [x] Validate every exact `PublicImportBatch` through
...
- [x] Write a failing regression for an exact but incomplete `Grid3D`.
- [x] Convert incomplete public model access to
  `CanonicalDocumentIntegrityError("incomplete public model value")`.
- [x] Preserve existing typed canonical errors, recursion handling, document
...
- [x] Write failing bridge tests for exact input contracts, selection,
...
- [x] Expose the initialized registry through internal
  `get_reader_plugin_registry()` without adding it to the public handle.
- [x] Implement `preflight_reader_plugins()` with a 65536-byte sniff prefix,
  canonical parameters, progress/cancellation, stable diagnostics, and exactly
  one staged internal batch per source.
- [x] Refactor source/revision staging into one shared helper while preserving
  `staged_reader_batch()` compatibility and parse-identity semantics.
- [x] Keep project mutation exclusively in the existing confirmed
...
- [x] Correct Task 1 live file state and require Blender 5.1.2 registration
  inventory before implementation.
- [x] Define module-by-module registration growth and Reader API handle
  lifecycle/rollback without clearing the initialized registry.
- [x] Route Quick Import through `get_reader_plugin_registry()` and
  `preflight_reader_plugins()`.
- [x] Correct `save_pre` semantics and define `ViewRecord` as UI/session-only
...
- [x] Run Blender 5.1.2 native validate/build, ZIP audit, isolated install,
  register/unregister/reload, and Reader API handle lifecycle.
- [x] Obtain independent specification and code-quality reviews and close all
  Critical, Important, and directly related Minor findings.
- [x] Record exact evidence in the active cursor and stop before
