...
| 优先级 | 内容 | 验证重点 |
| --- | --- | --- |
| P0 | 原子标量 named attribute、单一 instanced-arrow node group、当前帧轨迹、Volume/OpenVDB、mesh fallback、色标与单位 | 属性 ID 稳定；箭头数量不增加 Object；缓存可重建 |
| P1 | 振动动画、IR/UV-Vis、激发态、轨道选择、linked selection、表面顶点着色 | 光谱选择与三维对象使用同一 dataset/state ID |
