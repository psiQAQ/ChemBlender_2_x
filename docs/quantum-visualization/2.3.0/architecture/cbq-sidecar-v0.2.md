...
## Session service 与 cache 清理边界

`save_project_session()` 仅在 `.blend` 已有路径时把项目原子固化到同目录同名
`.cbq`，随后写入经验证的 project UUID、schema、相对 locator 和 manifest hash；
publication 与 Scene link 均成功后 session 才进入 `connected`。Scene 写入异常不
伪装为成功；rollback 完成时 Scene link 保持原值，rollback 自身失败时显式抛出
包含 residual keys 的 recovery error，Scene 可能保留部分值。两种情况下 session
均保留已发布 sidecar path 但进入 `invalid`，避免旧 manifest hash 与新 generation
并存时仍报告 `connected`。已完成且有效的 publication 保留供重试。

`verify_project_session()` 不修改 Scene；只有 `connected` 才由 session 接管本次
打开的 project，并先关闭被替换 project 的 lazy resources。`relink_project_session()`
先在临时 link document 中完整验证候选的 UUID、schema、manifest hash 与数组，再
一次写入 Scene 并更新 session。missing、mismatch、incompatible 和 invalid 返回
显式状态，失败不修改 Scene 或 session。

`.cbq/cache/derivation/` 与 `.cbq/cache/render/` 是可重建、非权威缓存，也是
`clear_derived_cache()` 唯一允许删除的 namespace。它不删除 sidecar root、
`manifest.json`、`arrays/`、`cache/parser/`、`cache/source/` 或未知 sibling。
清理前会扫描两个允许删除的目标树；其中任何 symlink、junction 或路径逃逸都会被
拒绝并保留。未识别的 cache sibling 不进入扫描，也不阻断或删除。无 cache 时操作
幂等；删除中途失败时返回已删除路径、失败路径和错误消息，不把部分清理报告为完整
成功。
