...
class StableReleaseReadinessTests(unittest.TestCase):
    def test_production_metadata_and_notes_describe_the_exact_stable_release(self):
        ...
        self.assertEqual("chemblender-2.5.0.zip", metadata.package_name)
        self.assertEqual("chemblender-2.5.0.sha256", metadata.checksum_name)
        self.assertEqual(
            "chemblender-2.5.0-windows-x64",
        ...
        self.assertIn(
            "[2.5.0]: https://github.com/psiQAQ/ChemBlender_2_x/"
            "releases/tag/v2.5.0",
            changelog,
        )
        self.assertIn(
