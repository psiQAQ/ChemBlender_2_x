...
## CBQ refactor started

Main 54ecf4c passed both remote workflows (extension-package 34223226599, optional-qc-core 34223226603). Verified remote main, annotated archive tag objects and peeled commits; removed two old local branches and origin workbench with its exact commit lease. Created feat/cbq-only-viewer. Preserved upstream and detached evidence worktree.

User approved cache scientific/Fermi/critic2 installation and repository uv init + uv venv for a future independent PyPI package. Both uv commands completed with existing CPython 3.12.13. Scientific 47-package smoke Passed; critic2 1.3.15 built and help/resource checks Passed. Fermi original 106-package lock imports failed on PyVista NORMALS API removal; compatibility lock repair and numerical smoke are in progress. No Blender or shared-user dependencies changed.

Relocated 136 modules into cbq_core and chemblender_prepare with AST-resolved imports. Root owns shared helpers/schema/transactions/build, converter agent CLI/GUI, Viewer agent UI boundary. Import and runtime validation remain in progress.

## 2026-09-08 计划调整：保留本地编辑，统一异步处理

按用户新增方案重写任务目标、阶段依赖和移除门槛，新增完整文档 docs/quantum-visualization/architecture/local-processor.md。先完成当前共享模块迁移的干净提交，再实施capabilities/worker/doctor、单exe路径和公共异步控制器。保留旧数值/CBQ工作，不覆盖当前大规模移动。

记录已完成但范围有限的共享42模块隔离导入、CBQ1.1专项、CLI真实输入及候选Blender生命周期验证。Fermi锁的兼容修复和实际数值探测已完成；不再把已批准依赖写为待批准。候选smoke-03为104members/0wheels基础Viewer验证，明确不能用于批准正式移除RDKit。纯Mesh编辑恢复、RDKit功能等价、前台响应、取消与冷启动门槛保持未完成。

同步当前active、依赖政策及新增架构决策；新增L1-L8不抢先接入。正式wheel保留和打包一致性修复与计划文档检查另行记录实际结果。

计划更新验证：19个本地链接/目录锚点Passed；staging回归1项Passed。实际保留wheel的stage含47个共享Python文件，逐文件源码hash一致；RDKit与Gemmi分别24,618,400和2,270,352字节，SHA256匹配原锁。manifest权限文字缩到Blender限制以内。仅恢复保留既有wheel，不安装依赖、不宣称旧编辑功能已恢复；新增控制器和操作未抢先接入。证据 .blend-analysis/2026-09-08-cbq-architecture-consolidation/local-processor-plan-verification.json。

迁移收尾继续：用户允许据实调整目标路径与优先级，保持功能/正确性/安全验收；8个测试模块改用共享模型与实际owner，保留类型身份、数值与来源断言。57项测试运行成功，5项可选依赖跳过；全量回归 migration-full-01.log 正在归因，C2尚未完成。

完整回归基线 migration-full-01：2450项，65 failures、539 errors、36 skips（修复前快照），失败清单已按原因存为 migration-full-01-failures.json。主要涉及旧UI/旧核心引用和schema断言，不能判定全量通过。随后8模块定向57项中52Passed/5skip；RDKit测试去除借用真实Blender共享site-packages的旧做法，改用当前已批准解释器环境，14项全部Passed，未改变数值断言或安装依赖。下一步继续按外置服务/保留本地编辑边界逐项修复，阶段仍未完成。


迁移实测修复：结构预设没有传入已经准备的 Topology，导致有键的 CBQ 显示为无键；现已在结构、轨迹、原子属性和关联光谱调用中使用既有拓扑选择逻辑。新增纯 Mesh 面板恢复元素选择、原子/键属性和局部坐标测量，CBQ 原数据保持不变；编辑 Apply 闭环仍待完成。Blender 5.1.1 原生私有 profile cbq-mesh-smoke-04 build/install/cold 全部 Passed，覆盖非平凡对象变换下的测量、芳香标记清除、保存重开、缓存重建与 Save As；数学和 staging 两项单测 Passed。MCP 本轮未连接，未将 CLI 验证冒充窗口截图验收。正式 RDKit/Gemmi wheels 继续保留。


迁移回归第二轮：migration-full-02.log 实际运行2462项，64 failures/290 errors/36 skips，仍未通过。随后将18个模块的冷启动导入与共享模型类型检查迁到实际owner，修复Worker mock目标路径与CBQ 1.1保存断言；旧1.0输入fixture保留用于升级验证。Reader conformance旧测试错误地把cp313 Blender wheels加载进项目Python3.12，改为使用当前已批准解释器依赖，12个required reader全部Passed；同步Reader API CLI文档入口。migration-targeted-03.log：244项，217 Passed/27 optional skips，无失败。周期测试改为提供明确数值对称矩阵，保留位点与ADP旋转数值断言，新增旧包仅source-sites可用而展开必须升级的检查；周期/CBQ升级26项全部Passed。未修改依赖、未恢复旧解析入口、未将专项通过标为完整回归通过。剩余主要为旧导入/导出UI契约迁移、生命周期注册契约和架构文档/打包验证。


注册生命周期迁移：22项契约全部Passed；删除旧测试harness中的Reader handle发布模拟，改为验证Viewer不触碰外来driver namespace/handler、拒绝其他package接管，并保留class/callback逆序清理、原异常notes、残留可重试等检查。发现并修复ui/cbq_import.py创建Scene属性后身份检查失败会残留属性的问题。新增原生RNA验证覆盖identity=None、probe异常、预占用及后续外来替换；cbq-registration-smoke-01私有Blender5.1.1 build/install/cold全部Passed，连同编辑、保存重开和缓存重建再次通过。更新架构导览三项实际模块职责，完整迁移清单仍待同步。正式依赖策略未改变；全量回归与稳定提交仍未完成。


Cube导出迁移：旧11项Blender导出契约转为真实外部export_service/CLI/GUI参数契约，保留精确结构/电荷/来源绑定、缺失/歧义电荷拒绝、非写入预览、数据集数值往返、损失确认、取消及原目标文件保护。发现CLI export与GUI默认为dataset 0会静默选取多dataset第一项，改为默认未选择；原始多dataset在预览和执行时均拒绝缺失index，明确index=1仍得到100..103真实fixture数值。CLI/导出29项运行28Passed/1optionalSkip。同步外部工具README；当前GUI仍有后续能力/统一路径改造待完成。完整回归未重新声明通过，下一步继续其他格式导出契约及完整架构清单。


晶体导出迁移：13项旧面板契约迁到外部服务/CLI/GUI，保留周期XYZ损失门槛、CIF preserve/normalized完整计划、派生结构来源、Partial提示、规范结构选取、目标文件保护与POSCAR数值往返。补齐迁移遗漏的CLI/GUI POSCAR注释/坐标/缩放/目标体积/源比例/约束/速度参数，以及GUI CIF模式和ExtXYZ缺失标记；隐藏格式选项不泄漏到其他格式。真实CsCl经CLI转换/预览/未确认拒绝/确认导出/重读，晶胞、坐标与原子数不变，负体积编码和约束省略符合显式参数。36项定向测试35Passed/1optionalSkip；完整回归仍未完成。

GUI export confirmation now resets after changes to any input field or input/companion text, and returns to preview without launching work. Native Tk regression and actual CLI conversion passed. POSCAR form requested size on this environment: 863x744 within default 900x850; no scrolling framework added. Targeted prepare/Cube/crystal suite: 44 run, 43 Passed, 1 optional IOData/GBasis skip. Full regression and C2 stable migration commit remain incomplete.

迁移第三轮完整回归 migration-full-03.log：2465项，33 failures/245 errors/36 skips，仍未通过；失败明细已保存。随后修正新包schema 1.1的链接/会话断言、Reader API实际外部源码路径、独立复制cbq_core+prepare且以Python -I运行的导入契约，以及Worker冷启动入口。相关109项全部Passed。旧UI测试setUp失败遗留fake bpy导致后续误报，5处补充addCleanup；两个实际失败setup重现验证没有再泄漏bpy。恢复误删的legacy检测/提取/迁移逻辑到chemblender_prepare/legacy，使用同一cbq_core；原晶体字段通过私有Blender临时历史RNA schema提取，退出移除属性，不加载旧插件。迁移核心26项Passed，原生Blender检测5项（含3个旧blend fixture）Passed；覆盖占据率/Uij/编辑坐标、非均匀变换诊断和临时属性清理。专用旧blend导出入口、旧View参数恢复、其余UI迁移和完整架构清单仍待完成，未形成稳定提交。

旧blend专用导出入口已接通：chemblender_prepare/legacy/__main__.py 由私有Blender --python调用，预览不写文件；export.py在临时目录写并校验project.cbq和migration.json后一起发布新目录，拒绝已有目标，发布前重验原文件hash，支持cancel marker。报告独立记录显示参数和CBQ manifest SHA-256，display_restore_status=recorded_only，尚未把参数自动应用到新View。三个hash锁定blend实测预览/导出/重读/重复目标保护/原文件不变全部通过；晶体占据率/Uij正确，经外部Gemmi upgrade补齐数值操作且坐标不变。取消、写入失败、源hash改变均不发布结果。迁移producer改为chemblender_prepare 0.1.0，revision legacy-migration-v2。导出+迁移核心28项Passed。docs/user/legacy-migration.md新增可执行流程、目录、晶体升级与显示恢复限制，历史面板步骤明确归档；架构导览同步两个新入口。完整回归尚未重跑，C2稳定提交和旧View自动恢复仍未完成。
...
2026-09-09：Molecular Mesh Editing 增加 Apply Mesh as New Structure。源 revision/atom mapping/键级/单位由既有冻结与 shared structure_edit 校验；共享 commit_session_batch 保存新 Structure/Topology/provenance 后切换，旧科学实体和对象保留。核心/CBQ事务/UI/编辑/文档67 Passed；进一步把原生成功路径改为实际 bpy.ops.chemblender.apply_mesh_edits 后2 Passed。原生 Blender5.1.1/Python3.13.9 私有 profile，阻止 rdkit/gemmi/chemblender_prepare import，验证元素/双键、新旧UUID和对象、对象平移不入科学坐标、CBQ重开、写入失败不发布、提交后View失败仍保留数据。测试使用私有会话及 browser property，session查找/通知为替身；不是完整注册或.blend冷重开验收。首次测试直接实例化bpy Operator失败，改实际ops成功路径、故障路径调用真实execute并收集report。MCP仍不可达，用户PID45116未动。架构导览和source-revisions操作说明已更新；未跑新全量，full10仍Failed（6 failures/116 errors/36 skips），C2未提交。

2026-09-09：Apply改为标准preset绑定；原生test_mesh_edit2 Passed。第一次候选安装cbq-mesh-apply-lifecycle-01因structure_edit绝对import失败，修复共享core相对import与stage审计后12项Passed；新候选02原生build/install/cold全部Passed。实际扩展bl_ext.user_default.chemblender，Blender5.1.1/Python3.13.9私有profile，完整注册/禁用/启用/reload，已保存场景Mesh Apply、旧结构/View保留、对象变换不入科学坐标、源CBQ移走、VDB缓存删除重建、冷启动派生View显式重建及SaveAs重开通过；无外部可选包，合成H2→O2编辑数据，不替代RDKit科学等价或所有量可视化验收。正式wheels保持不变。第十一轮完整回归2571项/100.141秒，5 failures/116 errors/36 skips，Failed；失败分组旧ImportPreview64、QuickImport22、wavefunction19、legacy路由13、旧blend迁移3，CLI故障已不在失败清单。日志migration-full-11及candidate02 install/cold.json。架构导览和source-revisions更新，diff check Passed；C2尚未稳定提交。

2026-09-09：迁移三项Scene所有权测试到CBQ入口，相关12 Passed。将旧QuickImport fatal-cleanup测试迁到外部GUI真实poll入口；先复现MemoryError被ValueError掩盖，再修复poll/start的致命异常优先级，增强二次清理重试与启动不拉起子进程断言，GUI生命周期9 Passed。CLI+GUI+CBQ UI共47项46 Passed/1 skipped；随后补start同根路径，GUI9再次Passed。未用删除安全要求消除失败；旧raw UI同名测试由新GUI契约替代。架构导览同步。未运行新全量或Blender，full11（2571项5 failures/116 errors/36 skips）仍是最新完整快照；不按单项通过推算全量成功，C2未提交。

2026-09-09：新增外部CliProcess启动失败测试，以真实临时目录/日志流、Popen故障注入覆盖FileNotFoundError/KeyboardInterrupt/GeneratorExit/MemoryError，验证启动前已持有资源、异常原对象、关闭日志/删除目录、含空格exe argv与无shell；GUI生命周期10 Passed。移除已由新边界覆盖的三项旧线程测试，保留尚未覆盖的取消/状态/预览要求。旧面板断言迁到真实draw函数（fake bpy/layout，仅面板路由契约），首次测试缺bmesh替身导致类未定义，补齐测试环境后相关20 Passed。无新增产品依赖或实现改动；未重跑全量或Blender，最新full11仍Failed，C2稳定提交及完整计划待完成。

2026-09-09：tests/blender_legacy_migrate.py新增external-only验收模式，复用快照函数，覆盖真实旧blend预览/取消/注入保存失败/成功导出，逐步核查对象清单、集合与可见性/custom properties、原Mesh坐标与边、场景CBQ链接、原blend字节均不变，失败目录清空、成功CBQ1.1/hash校验重开。tests.test_legacy_export新增用例在私有Blender5.1进程覆盖三个固定hash样例，1测试含3subtests Passed/4.624秒；Popen禁用autoexec，源文件只读。没有删除原legacy恢复测试，也未把recorded_only显示报告当恢复成功。产品代码无改动，全量仍full11 Failed；C2和完整计划未完成。
...
2026-09-09：历史Blender迁移回归仍调用已删除的旧UI迁移入口，导致三个哈希锁定fixture在测试初始化阶段失败。按当前架构将测试入口改为调用既有外部-only legacy export/reopen路径，保留fixture哈希、私有Blender profile和真实场景不变检查；尚未重跑该专项或完整回归。此改动仅校正测试边界，不恢复已移除的旧迁移UI。

2026-09-09：历史fixture专项修正后通过：`tests.test_legacy_migration_blender` 1项、3个Blender fixture、4.739秒；`tests.test_legacy_export` 3项、17.452秒，全部Passed。`git diff --check`通过。当前工作树共455个tracked变更、181个untracked文件，尚未分批暂存；提交操作曾被自动审批的usage limit拒绝，不能通过等价或间接方式绕过。
...
2026-09-09：L1统一入口完成。`chemblender-prepare`新增`capabilities --json`、`worker REQUEST RESULT --cancel-file CANCEL`和`doctor`，复用共享Worker Protocol v1及现有固定operation registry；版本化外部配置只接受wavefunction/scientific/fermi绝对Python路径和critic2路径，无shell、无pip/uv、无请求指定模块或callable。能力探测使用隔离子进程metadata并报告实际后端版本/缺失原因；复核中发现未配置路由会错误回退主Python并可能误报可用，已改为明确`environment_unavailable`并补红绿测试。专项5项及Worker/CLI/文档/仓库合同共121项Passed/1 optional IOData/GBasis skip。最终`uv build --offline`产物为wheel 550,427 bytes/SHA256 `742830f3c0f6fc9ba5f0788c26221f0e08ad48a1e799aaf2cd4303f0ed99345a`、sdist 892,323 bytes/SHA256 `fba04ef0532ba713974983c7314dd59b64cc5cbcf13c18d141935385091fc0ed`；wheel CRC、168 entries及`runtime.py`包含检查通过，`--no-deps`隔离安装launcher直接执行。批准的三套环境实测13 operations、22 readers全部可用；doctor 12项总状态Passed，critic2因配置为WSL ELF在Windows原生执行保持warning；跨GBasis环境worker进入协议并按缺参数返回预期`operation_failed`。证据目录为`.blend-analysis/2026-09-08-cbq-architecture-consolidation/{prepare-dist-l1-02,prepare-install-l1-02,worker-installed-l1-02}`。未改依赖、lock或项目环境；L1关闭，下一阶段严格进入L2 Blender异步控制器。

2026-09-09：L2实现与专项验证进行中。新增单一`processor_executable` Add-on Preferences、异步 capability/Worker v1 控制器、严格任务目录与request identity校验、可选进度、cancel marker及两秒后按owned PID终止进程树；非零退出码即使伪造success仍fail closed，取消后的晚success不提交。相位和源帧属性通过Blender timer合并为100 ms尾随本地预览，不启动处理程序。纯Python控制器/注册/仓库/文档/Scientific View专项共98项Passed；Blender 5.1.1私有profile实测注册、纯Mesh入口、三次相位更新仅应用末值、真实capability启动低于100 ms、卸载/reload通过，1项Passed/1.858秒。尚未运行扩展validate/build/安装版smoke及L2全量回归，故L2不标完成。

2026-09-09：L2关闭。Blender Add-on Preferences仅持久化一个绝对`processor_executable`，Test Processor异步显示处理程序/RDKit版本、13项operation可用状态及缺失原因；路径和能力缓存不进入Scene/CBQ。公共控制器使用无 shell子进程、私有任务目录、严格Worker v1/result identity/退出码校验、可选有界progress、cancel marker和两秒owned PID tree终止；失败/晚success不提交。phase/frame连续修改由Blender timer合并为100 ms末值本地预览。专项98项Passed；Blender 5.1.1源码与正式ZIP私有profile两项Passed，覆盖注册/卸载/reload、真实capability启动低于100 ms、处理程序未配置时纯Mesh入口仍可用。原生validate/build Passed；正式包29,705,048 bytes、110 members、SHA256 `be5da38cb49085bb21cf680a0a0caa9ee426bf08cf437587eeeafaff7859e53a`，CRC/安全路径/processor模块及锁定RDKit/Gemmi wheels审计Passed。首次full L2为2509项2个manifest hash锁失败；同步经批准的权限说明字节锁后两项专项Passed，复跑2509项0 failures/0 errors/36 skips、114.050秒。未改依赖/lock，未移除wheel；下一阶段严格进入L3现有重计算统一入口。

2026-09-09：L3实现进行中。`reader.parse@0.1`不再覆盖冻结输入CBQ，而是在任务目录生成独立`reader-result.cbq`；WorkerResult声明并由两端复验完整created-entity inventory。Blender新增统一operation staging/发布层，实际`xyz` reader经同一`chemblender-prepare`成功执行，严格验证后事务追加并选中，恢复原始来源locator/provenance，删除处理器任务目录后数组仍可用；篡改输出数组则拒绝且活动project不变。科学reader/Fermi面板和五项wavefunction按钮已接入同一modal控制器与单一全局可执行路径，首轮控制器/reader/注册/Viewer/wavefunction专项66项Passed/2 optional GBasis skips；尚未完成Blender源码/安装版真实操作、Fermi与wavefunction数值环境验证、validate/build及全量回归，因此L3保持未完成。

2026-09-09：L3继续收敛。Fermi发布层新增与CLI一致的原始绝对路径/hash归一化，避免持久CBQ引用即将删除的任务目录；operator卸载/会话清理现在可移除无context timer，且任务目录清理失败只告警、不覆盖已成功提交的科学结果。Project Browser draw改读无副作用snapshot，density-matrix分支不再显示无关轨道导出。尚待专项、真实Fermi、Blender与完整回归验证，L3仍未关闭。

2026-09-09：L3真实操作证据继续补齐。真实缓存VASP五文件经统一launcher完成Fermi surface，原始路径/hash归一化、surface数组及任务目录删除后读取通过（1项Passed/16.678秒）。真实FCHK先经`iodata_wavefunction`导入，再依次执行MO、electron density、density-matrix density、matrix ESP及orbital ESP五项统一Worker操作，全部生成finite `Grid3D`并保留唯一UUID（1项Passed/5.450秒）。Blender 5.1.1源码态首轮暴露“轨道元数据无scene representation时自动View异常覆盖已成功提交”缺陷，统一完成路径已降为warning；复测覆盖100 ms内invoke、reader、MO grid、自动View、保存重开、源移走及数组读取，1项Passed/3.390秒。尚待安装版、构建审计与全量回归，L3仍未关闭。

2026-09-09：L3关闭。正式`stage_viewer`链路完成validate/build，ZIP为29,715,245 bytes、111 members、SHA256 `14171c26b6f8a2ba24a68b4f4edc8313330410ce011f7f0af60b4e96adfa89d0`；CRC、安全路径、vendored `_cbq_core`、`ui/processor_operations.py`及锁定RDKit/Gemmi wheels审计Passed。直接源码build曾因缺vendored core在安装态失败，改回既有正式staging；安装smoke脚本顶层误导入源码`cbq_core`后按包模式选择共享核心命名空间，源码/安装版控制器4项均Passed/14.888秒。真实FCHK安装版完成reader→MO→View→保存重开→删除源文件后数组读取。首次full为2515项1 error，根因为Project Browser fake-bpy测试未清理/屏蔽新UI模块；按现有隔离清单修复，红绿顺序5项Passed/10.588秒，复跑full 2515项0 failures/0 errors/36 skips、134.046秒。未改依赖/lock，正式包继续保留wheel；下一阶段严格进入L4 RDKit操作闭环。

2026-09-09：L4实现进行中。用户要求的`uv build`首次因默认uv缓存目录初始化`os error 183`未进入构建，使用`uv build --no-cache`后Passed，生成wheel 550,997 bytes/SHA256 `8cdc8a7b975f7b14cab07982b3dd7cd9aa61a3cba9321aaae91f2ef3f324fdba`及sdist 892,628 bytes/SHA256 `19fae25e33ed37d0d9895915d9af1885889f9749ec5d7bdbd5136fd89a15c0ad`，未改依赖或lock。新增`molecule.smiles_to_3d/kekulize/optimize/energy/export@1`，复用既有RDKit模型重建、ETKDG和MOL/SDF/SMILES导出，势能以`kilocalorie_per_mole`标量PropertyDataset发布；capability仅在真实RDKit可用时声明这些操作。Blender Mesh面板通过同一异步控制器显式运行，未Apply草稿禁止重算，结果追加/选中且导出原子复制到用户路径。Worker、SMILES 3D及导出专项40项Passed/2.010秒，真实launcher控制器追加/选中/势能/导出/清理1项Passed/1.887秒。尚待补齐编辑Mesh→CBQ→新结果真实Blender闭环、L4完整分子矩阵、构建安装和全量回归，故L4不标完成。

2026-09-09：L4边界复核确认当前环境capability对6项`molecule.*`统一声明RDKit 2026.3.3，其中5项新操作均available；修复导出目标为空时`Path("")`被误当当前目录的问题，在启动层显式拒绝空值并清理已启动任务，新增最小回归。专项、Blender及完整回归尚待运行，L4保持进行中。

2026-09-09：L4源码态Blender 5.1.1真实闭环已Passed，覆盖苯CBQ导入/View、坐标Mesh编辑/Apply、外部UFF优化、势能、SDF导出、旧实体保留、保存重开、移走输入后数组/View仍可用。过程中定位并修复纯坐标/键编辑错误丢弃`atomic_identity`的问题：原子序号与一一映射不变时保留，增删/改元素继续fail closed；同时将空导出路径校验前移到worker启动前，UI不再先把空串转换成当前目录。现有SMILES专项已覆盖E/Z、芳香`[nH]`、显式AddHs、MMFF/UFF及多片段拒绝；尚待CLI真实operation、正式ZIP安装态、全量回归和文档，L4仍不标完成。

2026-09-09：L4关闭。统一CLI真实执行`derive molecule.smiles_to_3d@1`，以MMFF94/显式AddHs从苯输入生成独立CBQ并追加4个派生实体；6项`molecule.*` capability均报告实际RDKit 2026.3.3。最终正式staging validate/build Passed，ZIP为29,718,231 bytes、111 members、SHA256 `24c4f48fffe69edf5367657f8158ef6fe806986d1be69c2ae8564ea2b58bb3e5`，CRC、安全路径、vendored core、分子UI及锁定RDKit/Gemmi wheels审计Passed。Blender 5.1.1/Python 3.13.9全新安装profile出现`BLENDER_MOLECULE_OPERATIONS_PASSED`，覆盖Mesh→Apply→UFF→energy→SDF→保存重开→移走输入；最终全量2519项0 failures/0 errors/36 skips、137.914秒。首次full的1 error是未注册fake-bpy面板无条件加载processor模块，按注册态属性guard修复并专项7项Passed。未改依赖/lock，wheel按门槛继续保留；下一阶段严格进入L5专业分析。

2026-09-09：L5真实后端探测开始。已批准缓存中的critic2 1.3.15仅为WSL ELF，Windows直接配置仍不能执行；经显式`wsl.exe --exec`只读探测确认版本和Linux宿主。固定water HF/STO-3G WFX实际运行`AUTO`、`CPREPORT JSON`和`FLUXPRINT graph 2 text`成功，得到3个核CP、2个bond CP及4条有序梯度分支；同一WFX实际运行`NCIPLOT`生成40×40×40 RDG与signed-density Cube。当前这些是手工真实后端证据，尚未注册统一operation、完成CBQ发布或Blender显示，故L5保持未完成。

2026-09-09：L5统一operation已完成首批真实结果。`topology.qtaim@1`经配置的WSL critic2 1.3.15从真实water WFX发布2个TopologyGraph（5个CP、4条有序路径）；`grid.nci_fields@1`发布共享Structure及40×40×40的RDG/signed-density Grid3D对并通过NCI语义校验；`periodic.phonon@1`经科学环境phonopy 4.4.0从真实YAML/FORCE_SETS/BORN发布Structure与复数PhononModeSet。CLI按operation所需环境路由，Blender控制器冻结文件/hash、校验来源/provenance并事务追加。fake-bpy Project Browser因缺`AddonPreferences`测试桩出现1 error，补齐现有桩后32项专项Passed/0.402秒。尚待真实Blender显示、保存重开、架构文档、构建和全量回归，L5保持未完成。

2026-09-09：L5关闭。QTAIM/NCI/phonon三项统一operation已完成真实CLI、源码态和正式ZIP安装态Blender 5.1.1验证；控制器冻结原始文件/hash并仅归一化本次请求的来源，修复连续专业操作误写历史provenance导致UUID内容冲突。安装态实际运行critic2 1.3.15与phonopy 4.4.0，创建TopologyGraph、成对Grid3D与复数PhononModeSet，自动创建本地View，保存重开并删除CBQ/WFX/YAML/FORCE_SETS/BORN后数组和View仍可用，输出`BLENDER_PROFESSIONAL_OPERATIONS_PASSED`。正式staging validate/build Passed；ZIP 29,719,942 bytes、111 members、SHA256 `8b7b65b7b2a2d643614753e1d4b5c2cea8469ec94763f65879c928dffd4944f5`，路径/重复/CRC/vendored core/锁定RDKit与Gemmi wheels审计Passed。artifact资源预算因本阶段新增控制器资源超过旧基线而Failed，保留到L8用最终精确产物审计后更新，不放宽阈值。首次full为2521项3 failures，根因旧Fermi夹具未显式配置新环境路由；补齐指向当前解释器的测试配置后专项47项46 Passed/1 skip，最终full 2521项0 failures/0 errors/36 skips、139.705秒。未改依赖/lock，wheel继续保留；下一阶段严格进入L6删除前门槛。

2026-09-09：L6取消与进程所有权门槛开始。紧急审计发现75组`blender-mcp.exe`、其tool Python及uv Python配对链；按绝对可执行路径和PID终止74组遗留实例，保留Blender 5.1 PID 50440及自动恢复后固定PID的单一MCP链。当时连续三次采样和真实回归后复查均为1/1/1。Blender 5.1真实NCI 128^3任务通过公共modal控制器进入运行并取消：modal 0.0000088秒、运行态0.04908秒、取消确认0.32662秒，项目实体、timer和active-operation均不变/无残留；同一进程随后QTAIM/NCI/phonon均成功。沙箱内首次失败确认为`Wsl/Service/CreateInstance/E_ACCESSDENIED`环境限制，沙箱外真实主机Passed；旧L5 CBQ夹具的沙箱专属ACL通过复制到继承正常ACL的忽略证据目录规避，未改源文件权限。L6尚未关闭，继续等价、信任和生命周期矩阵。

2026-09-09：L6关闭。历史`78c2d8d`调用面与当前外部RDKit operation完成芳香/Kekulé、E/Z与手性、带电、多片段拒绝、AddHs、ETKDG、MMFF/UFF、势能及MOL/SDF/SMILES等价；103项专项全部Passed。阿司匹林21原子显式氢冷/热进程分别0.50311秒与0.48368/0.48732秒，冷启动额外0.01761秒；势能18.90977 kcal/mol按旧0.01容差一致。真实Blender NCI 128³门槛复测modal 0.0000078秒、running 0.01089秒、取消0.33582秒，并继续完成QTAIM/NCI/phonon；沙箱中的WSL权限限制仅精确skip，主机实测Passed。过期revision、路径穿越、绝对路径、symlink和hash篡改均拒绝且不发布；water/CH3/N2共40个View、61个数组完成Save As、整目录移动、删除VDB重建、移走处理器/输入/原始文件后的独立冷重开，正式examples哈希不变。实际Cycles 5.1.1 `water-homo` Research/Teaching两图320×240、32 samples非空且科学数组不变。首次full仅因沙箱WSL权限出现1 failure，加入环境精确skip且主机重测后，最终2524项0 failures/0 errors/37 skips、145.698秒。进程复核又发现Codex宿主直接再生3组及1组MCP链，累计按PID+绝对路径清理78组，最终保留Blender PID 50440和唯一链`48884→58844→43772`；未终止Codex、未删文件。正式包仍保留wheels，下一阶段严格进入L7。

2026-09-09：L7关闭。正式manifest、`dependencies.toml`、staging和extension-package CI一致移除RDKit/Gemmi/scientific wheels及共享PYTHONPATH；Blender继续使用Bundled NumPy，科学依赖只在外部prepare环境。快照资格验收`l7-wheel-free-05`的validate/build、staging字节比对、ZIP内容/CRC/vendored core、私有profile安装和冷启动均Passed：2,830,321 bytes、109 members、SHA256 `4dabb6dc463a740846eca72b239e875689bfdd07e390b06e34d82f63cfcbabb5`。`l7-wheel-free-viewer-02`进一步完成两轮reload、CBQ导入导出、纯Mesh编辑/Apply、Structure/Surface/Volume View、保存重开、删除源文件冷重建、Save As及三个`.blend`资产检查；独立冷依赖检查确认`find_spec("rdkit")`/`find_spec("gemmi")`为None、Bundled NumPy可用。相关89项契约Passed/1 skip；最终full 2525项0 failures/0 errors/37 skips、140.477秒。清理后再次按CIM绝对路径复核，仅Blender 5.1.1 PID 50440存活，重复MCP/Python链为0；连同为恢复transport终止的最后一组，累计清理79组遗留链，Codex侧MCP stdio仍为`Transport closed`，未伪装为产品失败。下一阶段严格进入L8最终精确包、artifact预算、逐物理量截图SOP和交付审计。

2026-09-09：L8交付文档开始收口。修复artifact预算脚本把合法空wheel inventory/empty approved set误判为无效的问题，并增加最小回归；10项专项Passed。最终精确资格验收`l8-final-01`已通过validate/build、staging、ZIP审计、私有profile安装与冷启动，产物2,830,321 bytes、109 members、SHA256 `317b7250e2a900d5f76a31c7d01e3f03afaebfb6d0cb6803a0e73ce98c575c13`；更新严格零增长预算后各区增长为0。`uv build --no-cache`生成wheel 559,417 bytes/SHA256 `eca1dd6dcafc37a520af541cc7661a163046a5ed3932551a35213eaacc8093c9`与sdist 897,243 bytes/SHA256 `82554df211cea58e2394f40f2824283d5a2292e89f59755e719ad1b76ad4c6f6`。SOP已迁移到单一processor_executable/Run Local Processor操作名，并按五层证据明确区分真实结果与Not Run；尚待生成离线HTML、最终资格重跑、全量回归和稳定提交，L8未关闭。

2026-09-09：L8补齐此前唯一缺失的真实ELF/LOL五层证据。critic2 1.3.15从真实water HF/STO-3G WFX生成40×40×40 ELF（0..1）与LOL（0..0.69383993498291）Cube；统一CLI分别以`elf`/`lol` preset发布`electron_localization_function`/`localized_orbital_locator`、`dimensionless` CBQ。无wheel安装扩展通过公开Preview/Import与Scientific View UI创建`Grid Volume`，两种物理量均输出Research/Teaching各2张320×240、32 samples真实Cycles图。随后把原CUBE和输入CBQ移到`removed-inputs`，两个独立Blender 5.1.1冷进程均从`.blend`持久sidecar恢复项目、校验数组极值、重建View、再次输出两张Cycles图并Save As；`find_spec`确认Blender环境无RDKit/Gemmi/prepare。证据在`.blend-analysis/l8-elf-lol`；尚待将SOP状态更新为Passed、重生成离线HTML、最终资格重跑、全量回归与稳定提交，L8仍未关闭。

2026-09-09：L8关闭，计划全部完成。`l8-final-02`从当前源码新快照完成Blender 5.1.1/Python 3.13.9原生validate/build、staging逐字节审计、私有profile安装/register/unregister/reload与独立冷启动，快照后源码无漂移；最终ZIP 2,830,321 bytes、109 members、SHA-256 `9ea6adcb84ff8c3e576652d9c140d111d9d509039e5da40d57a5bd404ed06708`，无wheel、artifact package/unpacked/code/resources/wheels/other增长全部为0，package-ci发行物五件套验证Passed。离线HTML从最终SOP重建，17 anchors、157 local links、0 remote resources；文档/预算/发行物专项81项Passed。最终全量2526项0 failures/0 errors/37 skips、147.228秒。`uv build --no-cache`发行物保持wheel 559,417 bytes/SHA-256 `eca1dd6dcafc37a520af541cc7661a163046a5ed3932551a35213eaacc8093c9`与sdist 897,243 bytes/SHA-256 `82554df211cea58e2394f40f2824283d5a2292e89f59755e719ad1b76ad4c6f6`。下一步只需提交本阶段收尾，不再有未完成计划项。

2026-09-09：最终进程复核发现Codex宿主再次生成3套重复MCP/Python链；按9个PID和绝对路径复验后全部终止，累计清理82组遗留链。最终CIM仅保留用户Blender 5.1.1 PID 50440，`blender-mcp.exe`与相关tool/uv Python均为0；未终止Codex或删除文件。
